...
def test_create_should_create_a_classroom_with_1_hour_duration():
    classroom = Classroom.create("mat class room", datetime(2020, 1, 2, 10, 0), 2, ClassroomSubject.MAT)
...
def test_classroom_should_have_a_duration_in_minutes():
    classroom = Classroom.create("machine beginners", datetime(2021, 5, 3), 2, ClassroomSubject.MACHINE_DUO, duration=Duration(MinuteTimeUnit(45)))

    assert classroom.duration == Duration(MinuteTimeUnit(45))


def test_classroom_should_be_scheduled():
    classroom = Classroom.create("machine", datetime(2020, 3, 19), 3, ClassroomSubject.MACHINE_TRIO, stop_date=datetime(2020, 6, 19))
...
def test_should_not_add_attendees_when_position_overflown():
    classroom = Classroom.create("machine", datetime(2020, 3, 19), 1, ClassroomSubject.MACHINE_PRIVATE, stop_date=datetime(2020, 6, 19))
...
def test_should_retrieve_next_classroom_session():
    classroom = Classroom.create("next session", datetime(2020, 9, 10, 10), 2, ClassroomSubject.MACHINE_DUO, stop_date=datetime(2021, 6, 10, 11))
...
def test_should_retrieve_next_classroom_session_with_duration():
    classroom = Classroom.create("next session", datetime(2020, 9, 10, 10), 2, ClassroomSubject.MACHINE_DUO, stop_date=datetime(2021, 6, 10, 10),
...
def test_should_retrieve_next_command_session_if_today_is_sunday_and_next_session_on_monday():
    classroom = Classroom.create("next session", datetime(2021, 8, 23, 10), 2, ClassroomSubject.MACHINE_DUO, stop_date=datetime(2022, 7, 12, 10))

    assert classroom.next_session()


def test_should_confirm_session_with_scheduled_time():
    classroom: Classroom = Classroom.create("classroom to be confirmed", datetime(2019, 6, 7, 10), 2,
                                            ClassroomSubject.MACHINE_DUO, duration=Duration(MinuteTimeUnit(45)))
...
def test_should_not_confirm_session_with_invalid_date_and_time():
    with pytest.raises(InvalidSessionStartDateException) as e:
        classroom: Classroom = Classroom.create("classroom to be confirmed", datetime(2020, 6, 1, 10), 2,
                                                ClassroomSubject.MACHINE_DUO, duration=Duration(MinuteTimeUnit(45)))
...
def test_session_in_range_should_return_sessions_until_classroom_stops():
    classroom: Classroom = Classroom.create("classroom", datetime(2020, 6, 1, 10), 2, ClassroomSubject.MACHINE_DUO, datetime(2020, 7, 13, 10, 45),
                                            duration=Duration(MinuteTimeUnit(45)))
