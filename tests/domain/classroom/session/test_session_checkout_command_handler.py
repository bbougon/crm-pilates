...
def test_should_not_refund_client_credits_on_two_consecutives_checkout_after_checkin(memory_event_store):
    client_repository, clients = ClientContextBuilderForTest() \
        .with_clients(1) \
        .with_client(ClientBuilderForTest().with_mat_credit(10).build()) \
        .persist(RepositoryProvider.write_repositories.client) \
    ...
    assert client.credits[0].value == 10
    assert e.value.message == "Cannot checkout as attendee is already registered (i.e not checked in)."
