...
def test_has_no_next_sessions():
    result: Tuple[NextScheduledSessions, Status] = NextSessionsCommandHandler().execute(GetNextSessionsCommand(datetime.now()))
...
def test_next_sessions_with_confirmed_sessions(memory_repositories):
    ...
    next_sessions: Tuple[NextScheduledSessions, Status] = NextSessionsCommandHandler().execute(GetNextSessionsCommand(datetime.now()))
